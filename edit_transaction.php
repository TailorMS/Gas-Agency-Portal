...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($transaction_date_err) && empty($transaction_type_err) && empty($transaction_id_err)){
        ...
        if($stmt = $mysqli->prepare($sql)){
            ...
            if($stmt->execute()){
                echo "<script>alert('Transaction updated successfully!'); window.location.href='transaction_details.php?transaction_id=" . urlencode($new_transaction_id) . "';</script>";
                exit;
            } else{
    ...
    <div class="page-container">
        ...
        <div class="profile-card">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?transaction_id=" . urlencode($original_transaction_id)); ?>" method="post">
                ...
                <p style="text-align: center; margin-top: 20px;"><a href="transaction_details.php?transaction_id=<?php echo urlencode($original_transaction_id); ?>">Back to Details</a></p>
            </form>
        </div>
    </div>
