...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($total_amount) && $total_amount >= 0){
        ...
        if($stmt = $mysqli->prepare($sql)){
            ...
            if($stmt->execute()){
                // Log the action
                $log_action = "Edited cash deposit ID: " . $deposit_id;
                ...
                echo "<script>alert('Deposit updated successfully.'); window.location.href='member_money_details.php?id=" . $deposit_id . "';</script>";
                exit;
            } else{
    ...
    <div class="page-container">
        ...
        <div class="profile-card">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . $deposit_id); ?>" method="post" id="cash-form">
                ...
                <a href="member_money_details.php?id=<?php echo $deposit_id; ?>" class="btn" style="display: block; width: fit-content; margin: 20px auto 0; background-color: #6c757d;" title="Back to Details"><i class="fas fa-arrow-left"></i></a>
            </form>
        </div>
    </div>
